...
export function BuildsSection({ projects }: { projects: Project[] }) {
  ...
  return <Section id="builds" eyebrow="Independent work" title="Things I build outside client work."><div className="mt-16 space-y-6">{featuredProjects.map(project => <ProjectCard key={project.title} project={project} />)}{secondaryProjects.length > 0 && <div className="grid gap-6 md:grid-cols-2">{secondaryProjects.map(project => <ProjectCard key={project.title} project={project} />)}</div>}</div></Section>;
}
